...
function getInput(req, res, next) {
	...
	if (!subscriber || (isObject(subscriber) && isEmpty(subscriber))) {
		return next(makeError("MissingParameter", "No subscriber specified.", 400));
	}
	// username always required
	if (!subscriber.email) {
		return next(makeError("MissingParameter", "Email required.", 400));
	}
	// password always required
	if (!subscriber.password) {
		return next(makeError("MissingParameter", "Password required.", 400));
	}
